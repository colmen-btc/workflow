...
- Template: `ai-sdlc-toolkit/templates/feature.md`
- Reference: `ai-sdlc-toolkit/samples/assessments/edi-express/features/asis-edi-express-01-f01-portal-provisioning.md`
...
```
FEATURE EVIDENCE GAP: <candidate feature title>  (Epic: <epic ID>)
...
1. **Read the template** at `ai-sdlc-toolkit/templates/feature.md`. Every `<!-- AGENT: ... -->` block is an authoring rule. Every `<!-- REPLACE: ... -->` block is a fill target. These are not text to copy.
...
8. **Assign feature IDs and slugs.** ID format: `<EPIC-ID>-F<NN>` (two-digit, left-padded). Slug format: `<epic-slug>-f<nn>-<kebab-case-behavior>`. File: `assessments/<system>/features/<slug>.md`. Sequence: happy-path features first → error/retry features → configuration/flag features.

9. **Draft each AS-IS feature** from the template with these adaptations:
   - **Header**: `Part of epic: <EPIC-ID> — <Epic Title>`. `Feature ID: <ID>`. `Slug: <slug>`. `Type: AS-IS (reference — describes shipped behavior)`.
...
10. **Update the parent epic's Features table** to use the stable IDs assigned in step 8. The epic file must reference the feature by `assessments/<system>/features/<slug>.md`.
...
- **Every feature maps to exactly one epic.** A feature cannot belong to two epics. If it crosses a boundary, place it in the most relevant epic and reference the other in `Depends On`.
- **Every feature cites evidence and carries a confidence level.** A behavior confirmed only in docs is Low and flagged.
- **IDs and slugs are stable.** The parity baseline, migration plan, and decommission plan reference features by ID.
...
- `ai-sdlc-toolkit/samples/assessments/edi-express/features/asis-edi-express-01-f01-portal-provisioning.md` — AS-IS feature with present-tense What, code-confirmed ACs, real Depends On, evidence note
...
- [ ] IDs (`<EPIC-ID>-F<NN>`) and slugs are unique and stable
